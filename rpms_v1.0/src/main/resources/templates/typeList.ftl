    ...
    <script>
        function delData() {
            ...
            if (confirm("您确定要删除id是" + str + "的数据吗")) {
                location.href = 'typeDelete2?ids=' + str;
            } else {
                alter("取消删除");
            }
        }
        ...
        function choosePageSize() {
            ...
            location.href = "queryTypeList?pageIndex=1&pageSize=" + v.toString();
...
<div id="center">
    ...
    <div id="main">
        <form name="form1" method="post" action="queryTypeList2?pageIndex=1&pageSize=3" onsubmit="return checkkey()">
            <span class="style3">查询条件：</span>
            <select name="oneway">
                <option value="0">--请选择--</option>
                <option value="1">按物资类名</option>
            </select>
            <input type="text" name="typeName" value="请输入关键字" id="txt1" onfocus="selectText();"/>
            <input type="submit" name="Submit" value="查询"/>
        </form>
        <form name="form2" method="post" action="#" onsubmit="return befSubmit();">
            <div id="table">
                <table id="ec_table">
                    <caption>物资类别信息表</caption>
                    <thead>
                    <tr>
                        <th>选中</th>
                        <th onclick="sortTable('ec_table',1,'string')">类别名</th>
                        <th>修改</th>
                    </tr>
                    </thead>
                    <tbody>
                    <#list queryList as type>
                        <tr onmouseover="this.className='lightup'" onmouseout="this.className='lightdown'">
                            <td><input name="ids" type="checkbox" value="${type.typeId}"/></td>
                            <td>${type.typeName}</td>
                            <td><a href="typeUpdate2?id=${type.typeId}">edit</a></td>
                        </tr>
                    </#list>
                    </tbody>
                    <tfoot>
                    <table>
                        <tr>
                            <td width="60">
                                <#if pageIndex!=1>
                                    <a href="queryTypeList?pageIndex=${pageIndex-1}&pageSize=${pageSize}">上一页</a>
                                </#if>
                            </td>
                            <td width="60">
                                <#if pageIndex!=totalPage>
                                    <a href="queryTypeList?pageIndex=${pageIndex+1}&pageSize=${pageSize}">下一页</a>
                                </#if>
                            </td>
                            ...
                            <td>
                                <ul>
                                    <li><input name="addnews" type="button" value="添 加"
                                               onclick="window.location='typeAdd'" class="button"
                            ...
                            <td>
                                <ul>
                                    <li><input name="delete" type="button" value="删 除" class="button"
                                               onclick="delData();" onmouseout="this.className='button';"
                                               onmouseover="this.className='buttondown';"/></li>
                                </ul>
                            </td>
                        </tr>

...
<head>
    ...
    <script>
        function delGoods() {
            ...
            if (confirm("您确定要删除id是" + str + "的数据吗")) {
                location.href = 'delGoods?ids=' + str;
            } else {
                alter("取消删除");
            }
        }
        ...
        function choosePageSize() {
            ...
            location.href = "queryGoodsList?pageIndex=1&pageSize=" + v.toString();
...
<div id="center">
    ...
    <div id="main">
        <form name="form1" method="post" action="queryGoodsList2?pageIndex=1&pageSize=3" onsubmit="return checkkey()">
            <span class="style3">查询条件：</span>
            <select name="oneway">
                <option value="0" selected>--请选择--</option>
                <option value="1">按物资名称</option>
                <option value="2">按数量</option>
                <option value="3">按单价</option>
                <option value="4">按物资类别</option>
                <option value="5">按入库时间</option>
                <option value="6">按出库时间</option>
                <option value="7">物资状态</option>
            </select>
        ...
        <form name="form2" method="post" action="#" onsubmit="return befSubmit();">
            <div id="table">
                <table id="ec_table">
                    <caption>物资管理表</caption>
                    <thead>
                    <tr>
                        <th>选中</th>
                        <th onclick="sortTable('ec_table',1,'string')">物资名称</th>
                        <th onclick="sortTable('ec_table',2,'string')">数量</th>
                        <th onclick="sortTable('ec_table',3,'float')">单价</th>
                        <th onclick="sortTable('ec_table',4,'string')">物资类别</th>
                        <th onclick="sortTable('ec_table',5,'string')">入库时间</th>
                        <th onclick="sortTable('ec_table',6,'string')">出库时间</th>
                        <th onclick="sortTable('ec_table',7,'string')">物资状态</th>
                        <th>修改</th>
                    </tr>
                    </thead>
                    <tbody>
                    <#list queryList as goods>
                        <tr onmousemove="this.className='lightup';" onmouseout="this.className='lightdown';">
                            <td><input name="ids" type="checkbox" value="${goods.materialId}"/></td>
                            <td>${goods.materialName}</td>
                            <td>${goods.materialCount}</td>
                            <td>${goods.materialPrice}</td>
                            <td>${goods.typeId}</td>
                            <td>${goods.materialDate}</td>
                            <td>${goods.materialOutDate}</td>
                            <#--<td>${goods.materialDate?String('yyyy-MM-dd HH:mm:ss')}</td>
                             <td>${goods.materialOutDate?String('yyyy-MM-dd HH:mm:ss')}</td>-->
                            <td>在库</td>

                            <td><a href="updateGoods2?id=${goods.materialId}">edit</a></td>
                        </tr>
                    </#list>
                    </tbody>
                    <tfoot>
                    <table>
                        <tr>
                            <td width="60">
                                <#if pageIndex!=1>
                                    <a href="queryGoodsList?pageIndex=${pageIndex-1}&pageSize=${pageSize}">上一页</a>
                                </#if>
                            </td>
                            <td width="60">
                                <#if pageIndex!=totalPage>
                                    <a href="queryGoodsList?pageIndex=${pageIndex+1}&pageSize=${pageSize}">下一页</a>
                                </#if>
                            </td>
                            ...
                            <td>
                                <ul>
                                    <li><input name="addnews" type="button" value="添 加"
                                               onclick="window.location='insertGoods'" class="button"
                            ...
                            <td>
                                <ul>
                                    <li><input name="delete" type="button" value="删 除" class="button"
                                               onclick="delGoods();" onmouseout="this.className='button';"
                                               onmouseover="this.className='buttondown';"/></li>
                                </ul>
                            </td>
                        </tr>

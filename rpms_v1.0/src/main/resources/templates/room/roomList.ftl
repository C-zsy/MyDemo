...
<head>
    ...
    <script type="text/javascript">
        function pageData(pageIndex, pageSize) {
            location.href = "queryRoomList?pageIndex=" + pageIndex + "&pageSize=" + pageSize;
        }
        ...
        function fun() {
            ...
            location.href = "queryRoomList?pageIndex=1&pageSize=" + val.toString();
        }
    </script>
</head>
...
<div id="center">
    ...
    <div id="main">
        <form name="form1" method="post" action="roomSearchList" onsubmit="return checkkey()">
            <span class="style3">查询条件：</span>
            <select name="oneway">
                <option value="0">请选择类型</option>
                <option value="1">楼栋名</option>
                <option value="２">房间名</option>
        ...
        <form name="form2" method="post" action="roomDelete" onsubmit="return befSubmit();">
            <div id="table">
                <table id="ec_table">
                    ...
                    <#list queryList as list>
                        <tr class="lightdown" onmousemove="this.className='lightup';"
                            ...
                            <td><input name="checkone" type="checkbox" value="${list.roomId}"/></td>
                            ...
                            <td>${list.roomUse}</td>
                            <td>${list.roomArea}</td>
                            <td>${list.roomPercent}</td>

                            <td><a href="roomUpdate?id=${list.roomId}">修改</a></td>
                    ...
                    <tr>
                        <td colspan="9">
                            <ul>
                                <li>共${totalCount}条信息</li>
                                <li>当前第${pageIndex}页</li>
                                <li>
                                    <input type="button" value="首页" onclick="pageData(1, ${pageSize});">
                                </li>
                                <li>
                                    <#if pageIndex != 1>
                                        <input type="button" value="前页" onclick="pageData(${pageIndex-1}, ${pageSize});">
                                    </#if>
                                </li>
                                <li>
                                    <#if pageIndex != totalPage>
                                        <input type="button" value="后页" onclick="pageData(${pageIndex+1}, ${pageSize});">
                                    </#if>
                                </li>
                                <li>
                                    <input type="button" value="末页" onclick="pageData(${totalPage}, ${pageSize});">
                                </li>
                                <li><span>转到</span>
                                    <select name="" id="pagenumber" onchange="build();">
                                        <option value="1">第${pageIndex}页</option>
                                    </select>
                                </li>
                                <li>
                                    每页最大条数：
                                    <select id="test" onchange="fun();">
                                        <option value="0">${pageSize}</option>

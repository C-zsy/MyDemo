...
<div id="center">
    ...
    <div id="main">
        <form name="form1" method="post" action="searchRolePower.html" onsubmit="return checkkey()">
            <#--            <div id="index">-->
            <span class="style3">搜索条件</span>
            <select name="oneway">
                <option value="0">--请选择--</option>
                <option value="1">按角色</option>
                <option value="2">按权限</option>
            </select>
            <input type="text" name="buildKey" value="请输入关键字" id="txt1" onfocus="selectText();"/>
            <input name="search" type="submit" value="搜索"/>
            <#--            </div>-->
        </form>
        <form name="form2" method="post" action="rolePowerDelete" onsubmit="return befSubmit()">
            <div id="table">
                <table id="ec_table">
                    <caption>角色权限表</caption>
                    <thead>
                    <tr>
                        <th>选中</th>
                        <th onclick="sortTable('ec_table',1,'string')">角色名</th>
                        <th onclick="sortTable('ec_table',2,'string')">权限</th>
                        <th>修改</th>
                    </tr>
                    </thead>
                    <tbody>
                    <#list rolePowerList as list>
                        <tr onmouseover="this.className='lightup'" onmouseout="this.className='lightdown'">
                            <td><input name="checkone" type="checkbox" value="${list.roleId}"/></td>
                            <td>${list.roleName}</td>
                            <td>${list.roleResource}</td>
                            <td><a href="rolePowerUpdate?id=${list.roleId}&Name=${list.roleName}&Resource=${list.roleResource}">修改</a></td>
                        </tr>
                    </#list>
                    </tbody>
                    ...
                    <tr>
                        <td colspan="4">
                            <ul>
                                <li><input name="addnews" type="button" value="添加权限 " class="button"
                                           onmouseout="this.className='button';"
                                           onmouseover="this.className='buttondown';"
                                           onclick="window.location='rolePowerAdd'"/></li>
                                <li><input name="delete" type="submit" value="删除权限 " class="button"
                                           onmouseout="this.className='button';"
                                           onmouseover="this.className='buttondown';"/></li>
                                <li><input name="st" type="button" value="查看权限" class="button"
                                           onmouseout="this.className='button';"
                                           onmouseover="this.className='buttondown';"
                                           onclick="window.location='powerList'"/></li>
                            </ul>
                        </td>
                    </tr>
                    </tfoot>
